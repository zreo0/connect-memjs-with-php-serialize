...
var PHPSerializeTool = require('./php-serialize');
...
module.exports = function(session){
    ...
    */
    ...
    function MemcachedStore(options) {
        ...
        if (!options.client) {
            ...
            if (options.isUsePHPSession) {
                this.isUsePHPSession = options.isUsePHPSession;
            }
    ...
    */
    ...
    /**
    * 是否使用 php session 默认的序列化格式
    * @api private
    */
    MemcachedStore.prototype.isUsePHPSession = false;
    ...
    */
    ...
    MemcachedStore.prototype.get = function(sid, fn) {
        ...
        this.client.get(sid, function(err, data) {
            ...
            try {
                if (that.isUsePHPSession) {
                    data = PHPSerializeTool.unserializeSession(data.toString());
                    if (!data.cookie) {
                        data.cookie = Object.assign({}, {
                            originalMaxAge: null,
                            expires: null,
                            httpOnly: true,
                            path: '/',
                    ...
                    // 这里为了适配 fastify 的 session 校验
                    if (!data.sessionId) {
                        data.sessionId = originSid;
                    }
                } else {
    ...
    */
    ...
    MemcachedStore.prototype.set = function(sid, sess, fn) {
        ...
        var sess = this.isUsePHPSession ? PHPSerializeTool.serializeSession(JSON.parse(JSON.stringify(sess))) : sess = JSON.stringify(sess)
